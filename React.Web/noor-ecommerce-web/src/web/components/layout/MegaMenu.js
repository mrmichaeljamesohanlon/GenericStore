...
const MegaMenu = () => {
    ...
    useEffect(() => {
        ...
        const getDataInUseEffect = async () => {
            ...
            if (categoriesResponse != null && categoriesResponse.data != null) {
                const categoriesData = JSON.parse(categoriesResponse.data.data);
                const uniqueCategories = [...new Map(categoriesData.map(item => [item.CategoryID, item])).values()];
                await setCategoriesList(uniqueCategories);
    ...
    const childrenByParent = CategoriesList.reduce((map, item) => {
        if (item.ParentCategoryID != null) {
            if (!map[item.ParentCategoryID]) map[item.ParentCategoryID] = [];
            map[item.ParentCategoryID].push(item);
        }
        return map;
    }, {});

    const sortedParentCategories = [...CategoriesList]
        .filter(item => childrenByParent[item.CategoryID]?.length > 0)
                    ...
                    <div className="container">
                        <nav className="navbar navbar-expand-md navbar-light">
                            <Link to={`/${getLanguageCodeFromSession()}/`} className="navbar-brand">
                                <img src={logoDataUri} width={155} height={41} alt="logo" />
                            </Link>
                                    ...
                                    <li className="nav-item megamenu">
                                        ...
                                        <ul className="dropdown-menu">
                                            <li className="nav-item">
                                                <div className="container">
                                                    <div className="row">
                                                        {sortedParentCategories.map((item, idx) => {
                                                                    ...
                                                                    <>
                                                                        <div className="col">
                                                                            ...
                                                                            <ul className="megamenu-submenu">
                                                                                {[...(childrenByParent[item.CategoryID] || [])].sort((a, b) => (a.Name ?? '').localeCompare(b.Name ?? '')).map((elementChild, idxChild) => {
