...
import { buildProductDetailUrl, getProductIdValue, makeImageUrl, makeProductShortDescription } from "../../../helpers/ConversionHelper";
...
const ProductBox = ({ item, hoverEffect, layout, ProductDetailPageForceUpload }) => {
    ...
    const reloadProductDetail = (_productId, _categoryName, _productName) => {
        ...
        let productDetailUrlFromForceReload = buildProductDetailUrl(_productId, _categoryName, _productName, getLanguageCodeFromSession());

        if (ProductDetailPageForceUpload != undefined && ProductDetailPageForceUpload != null && ProductDetailPageForceUpload == true && getProductIdValue(_productId) > 0) {
                ...
                <div className="product-imgbox">
                    <div className="product-front">
                        ...
                        {(() => {
                            ...
                            let urlViewDetailImage = buildProductDetailUrl(item, item.CategoryName, item.ProductName, getLanguageCodeFromSession());
                            return (
                                <>
                                    <Link to={urlViewDetailImage} onClick={() => reloadProductDetail(item, item.CategoryName, item.ProductName)}>

                                        {
                                            item?.ProductImagesJson?.slice(0, 1)?.map((img, imgIdx) =>
                                                <>

                                                    <Img src={makeImageUrl(adminPanelBaseURL, img.AttachmentURL)} className="img-fluid" alt="product" />
                                                </>
                    ...
                    <div className={`product-icon ${hoverEffect}`}>
                        ...
                        <a
                            onClick={(e) => {
                                e.preventDefault();
                                HandleCustomerWishList(productId, item.ProductName, item.Price, item.DiscountedPrice, item.DiscountId, item.IsDiscountCalculated, item.CouponCode, (item?.ProductImagesJson[0]?.AttachmentURL != undefined ? item?.ProductImagesJson[0]?.AttachmentURL : ""))
                            }}
                        >
                            <i className="ti-heart" aria-hidden="true"></i>
                        </a>
                        <Link to={buildProductDetailUrl(item, item.CategoryName, item.ProductName, getLanguageCodeFromSession())} title="Quick View" >
                            <i className="ti-search" aria-hidden="true"></i>
                        </Link>
                        <a href="#" title="Compare"
                            onClick={(e) => {
                                e.preventDefault();
                                handleCompareList(productId)
                            }}
                        >
