...
import { buildProductDetailUrl, makeAnyStringLengthShort, makeImageUrl, makeProductShortDescription } from '../../../helpers/ConversionHelper';
                            ...
                            PopularProductsList?.map((item, idx) =>
                                <div className="media">
                                    ...
                                    onClick={(e) => {
                                        ...
                                        forceLoadProduct(buildProductDetailUrl(item, item.CategoryName, item.ProductName, getLanguageCodeFromSession()));
                                    }}
                                    >
                                        
                                    {
                                        item?.ProductImagesJson?.slice(0, 1).map((img, imgIdx) =>
                                            <>
                                                 <Media className="img-fluid " src={makeImageUrl(adminPanelBaseURL, img.AttachmentURL)} alt="side popular product" />
