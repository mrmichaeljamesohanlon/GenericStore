...
import { buildProductDetailUrl, makeImageUrl, makeProductShortDescription } from "../../../helpers/ConversionHelper";
...
const productCollection = [
  {
    ...
    "id": 12,
    "title": "boho tops",
    "description": "A quality product from our collection.",
    "type": "fashion",
    "brand": "nike",
    "category": "Women",
    "price": 129,
                        ...
                        <Slider asNavFor={nav1} ref={(slider) => (slider2.current = slider)} {...settings}>
                          {HotProduct &&
                            HotProduct.ProductImagesJson?.map((img, i) => {
                              return (
                                <div key={i}>
                                  <Media src={makeImageUrl(adminPanelBaseURL, img.AttachmentURL)} alt="hot-deal" className="img-fluid  " />
                                </div>
                              );
                            })}
                        </Slider>
                        ...
                        <div className="hot-deal-center" style={{ display: "block" }}>
                          <div>
                            <div>
                              ...
                              <Link to={buildProductDetailUrl(HotProduct, HotProduct.CategoryName, HotProduct.ProductName, getLanguageCodeFromSession())}>
                                <h5 >{makeProductShortDescription(HotProduct.ProductName, 33)}</h5>
                              </Link>
...

                                {
                                  item?.ProductImagesJson?.slice(0, 1)?.map((img, imgIdx) =>
                                    <>
                                      ...
                                      <Media src={makeImageUrl(adminPanelBaseURL, img.AttachmentURL)}
                                        className="img-fluid"
                                        alt="banner"
