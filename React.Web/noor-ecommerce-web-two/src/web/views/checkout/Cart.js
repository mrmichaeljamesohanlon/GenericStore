...
import { getSanitizedCartItems, syncCartStorage } from '../../../helpers/CartHelper';
import { buildProductDetailUrl, makeImageUrl, makePriceRoundToTwoPlaces, makeProductShortDescription } from '../../../helpers/ConversionHelper';
...
const Cart = () => {
    ...
    const cartItemsSession = getSanitizedCartItems(cartJsonDataSession);
    ...
    const handleSubtractQuantity = (ProductId) => {
        ...
        if (qty > 1) {
            ...
            //--store in storage
            syncCartStorage(cartItemsSession);
            //store in redux
    ...
    const handleRemove = (ProductId) => {
        ...
        //--store in storage
        syncCartStorage(updatedProductsList);
        //store in redux
                                                ...
                                                <tr>
                                                    <td>
                                                        <a href="#" onClick={(e) => e.preventDefault()}>
                                                            <img src={item.ProductImagesJson?.[0]?.AttachmentURL != undefined ? makeImageUrl(adminPanelBaseURL, item.ProductImagesJson?.[0]?.AttachmentURL) : ""} alt="cart" className=" " />
                                                        </a>
                                                    </td>
                                                    <td>
